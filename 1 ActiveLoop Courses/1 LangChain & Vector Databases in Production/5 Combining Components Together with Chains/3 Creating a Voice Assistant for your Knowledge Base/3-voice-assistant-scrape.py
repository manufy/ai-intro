...
# To prepare the collected text for embedding into our vector database, 
# we load the content from the file and split it into separate documents 
# using the load_docs() function. To further refine the content, 
# we split it into individual chunks through the split_docs(). 
# Here we’d see a Text loader and text_splitter in action. 
# The instructiontext_splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=0)
# creates an instance of a text splitter that splits the text into chunks based on characters. 
# Each document in docs is split into chunks of approximately 1000 characters, 
