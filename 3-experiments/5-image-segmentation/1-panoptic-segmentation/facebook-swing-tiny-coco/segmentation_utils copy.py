...
def segment_image_from_url(image_url):
    response = requests.post(API_URL, headers=headers, json={"url": image_url})
...
def overlay_masks_on_image(image, segments, transparency=0.4):
    ...
    # ONLY IF HTTP original_image = Image.fromarray(image).convert("RGBA")
...
def segment_and_overlay_results(image_path, api_token, model):
    ...
    try:
       ...
       #     return None, []
        ...
        processed_image = overlay_masks_on_image(image_path, segments)
